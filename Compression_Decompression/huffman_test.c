...
//   Huffman_node_t node3 = { .frequency = 5, .by_ascii = 'C' };    ASSERT(huffman_frequency_compare(&node1, &node2) < 0, "node1 should be less than node2");
...
//    initialize_nodes(nodes);    const U_08 data[] = "AABBBCCCC";
...
//    U_32 output_size;    
//    huffman_encode(input, output_buffer, input_size, &output_size);    ASSERT(output_size > 0, "Output size should be greater than 0");
