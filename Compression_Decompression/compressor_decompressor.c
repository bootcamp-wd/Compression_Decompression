...
{
    log_debug("Starting Process_file function");
    ...
    {
        log_info("Decompression mode activated");
    ...
    {
        free(input_buffer_p);
        log_trace("Freed input buffer memory");
    }

    log_trace("Writing output file");
    ...
    if (compressed_data_buffer_p)
    {
        ...
        log_trace("Freed compressed data buffer memory");
    }
    ...
    U_32* output_size, U_32 compress_level)
{
    ...
    *output_buffer = (U_08*)malloc((*output_size) * sizeof(U_08));
    ...
    {
        log_fatal("Memory allocation failed in compress_data");
        free(lz77_output);
        lz77_output = NULL;
        free(huffman_output);
        huffman_output = NULL;
    ...
    log_debug("Allocated memory for output buffer");
    ...
    **(U_32**)output_buffer = input_size;
    ...
    log_trace("Compressed data written to output buffer");

    if (lz77_output)
    {
        free(lz77_output);
        lz77_output = NULL;
        log_trace("Freed LZ77 output memory");
    }
    /* if (huffman_output)
     {
         free(huffman_output);
         huffman_output = NULL;
         log_trace("Freed Huffman output memory");
     }*/
}
...
{
    log_debug("Starting Decompress_data function");
    ...
    *output_size = output.original_size;
    ...
    if (huffman_output == NULL)
    {
        log_fatal("Memory allocation failed in decompress_data");
        exit(1);
    }
    log_debug("Allocated memory for Huffman output during decompression");

    U_32 input_size_huffman = input_size - sizeof(U_32);
    U_32 output_size_huffman = 0;

    log_trace("Decoding data with Huffman");
    ...
    log_debug("Allocated memory for LZ77 output during decompression");

    log_trace("Decoding data with LZ77");
    ...
    *output_buffer = (U_08*)malloc(*output_size * sizeof(U_08));
    ...
    {
        log_fatal("Memory allocation failed for output_buffer");
        free(huffman_output);
        huffman_output = NULL;
        free(lz77_output);
        lz77_output = NULL;
    ...
    log_debug("Allocated memory for output buffer after decompression");

    // Correct the memcpy call
    memcpy(*output_buffer, lz77_output, *output_size);
    log_trace("Data copied to output buffer");
