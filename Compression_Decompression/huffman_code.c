...
{
    log_debug("Starting Huffman_frequency_compare function");
    ...
    log_debug("Comparing frequencies: %d and %d", a->frequency, b->frequency);
...
{
    log_debug("Starting Huffman_ascii_compare function");
...
{
    log_debug("Starting Initialize_nodes function");
...
//    }
//     log_trace("Metadata stored successfully. Tree length: %d", metadata->tree_length);
//}
...
{
    log_debug("Starting Encode_data function");
    ...
    log_trace("Data encoding completed. Compressed data bits: %ld", *compressed_data_bit_index);
}

void finalize_compressed_data(U_08* compressed_data, long compressed_data_bit_index)
{
    log_debug("Starting Finalize_compressed_data function");
    ...
    log_trace("Compressed data finalization complete.");
}
...
{
    log_debug("Starting Huffman_encode function");
    ...
    *output_size = 0;
    if (input_size == 0)
    {
        log_warn("Input size is zero. Nothing to encode.");
        return;
    }
    ...
    *output_size = (compressed_data_bit_index + 7) / 8 + 1 + metadata_size;  // +1 for the remaining bits byte
    ...
    log_info("Huffman encoding completed. Output size: %d", *output_size);
...
{
    log_debug("Starting Huffman_build_tree function");
    ...
    while (min_parent - nodes < NODES_IN_TREE && flag)
    {
        ...
        else
        {
            ...
            log_debug("Pushing parent node with frequency: %d at index: %d", parent.frequency, current_parent_index);
            priority_queue_push(nodes, &current_parent_index, &parent);
            log_debug("After push, nodes[%d].frequency = %d", current_parent_index - 1, nodes[current_parent_index - 1].frequency);
        }
    }
    *last_index = current_parent_index - 1;

    log_info("Huffman tree built successfully. Last index: %d", *last_index);
...
{
    log_debug("Starting Generate_codes_recursive function");
    generate_codes_recursive(root, 0, 0);
    log_trace("Huffman codes generated successfully.");
}
...
void generate_codes_recursive(Huffman_node_t* node, U_32 current_code, U_32 current_length)
{
    ...
    if (!node->left && !node->right) // Leaf node
    {
        ...
        if (node->code == NULL)
        {
            log_fatal("Memory allocation failed for Huffman code!");
            exit(1);
        }
        ...
        log_debug("Generated code for ASCII %d with length %d", node->by_ascii, current_length);
        return;
    }
    else {
...
{
    log_debug("Starting Huffman_free_tree function");
    ...
    log_trace("Huffman tree freed successfully.");
}
...
{
    log_debug("Starting Huffman_decode function");
    ...
    *output_size = 0;
    ...
    U_32 data_length = *input_size - meta_data_size;
    ...
    U_32 bit_index = 0;
    U_32 last_bytes = 2;
    U_32 data_bits_length_without_lasts_bytes = (data_length - last_bytes) * NUM_BITS_IN_BYTE;
    ...
    while (bit_index < data_bits_length_without_lasts_bytes)
    {
        ...
        log_debug("Decompressed byte: '%c' (ASCII: %d)", decompressed_byte, decompressed_byte);
    ...
    log_info("Huffman decoding completed. Output size: %d", *output_size);
}
...
{
    log_debug("Starting Find_ascii_in_tree function");
    ...
    while (current_node && current_node->left != 512 && current_node->right != 512)
    {
        ...
        if (((*bit_index) % NUM_BITS_IN_BYTE) == 0)
        {
            (*input_pointer)++;
        }
    }
    log_debug("Found ASCII: %c (ASCII: %d)", current_node->by_ascii, current_node->by_ascii);
...
{
    log_debug("Starting Find_ascii_last_byte function");
    ...
    for (U_32 i = index_bit_in_byte; i < sum_bits_in_last_byte;)
    {
        ...
        log_debug("Found last byte ASCII: %c (ASCII: %d)", current_node->by_ascii, current_node->by_ascii);
